...
export const IntegrationSecretsTool: Tool = {
  ...
  description: `Resolve the calling user's secret-mechanism integrations into a flat env map. Called whenever the agent needs an API key the user manages from the /integrations admin UI — OpenAI, GitHub, Stripe, etc.

Returns { env: { ENV_NAME: "value", … } }. Empty object if the user has no matching connected integration — in that case, ask them to open /integrations and connect the service.
  ...
  async execute(params: unknown, ctx: ToolContext): Promise<unknown> {
    ...
    if (!userId) {
      return {
        env: {},
        error:
          "ctx.from is empty — integration_secrets is only available in chat-driven sessions where the user identity is known.",
      }
    }
